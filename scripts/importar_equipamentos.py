...
# --- Dados de equipamentos solicitados (112 municípios) ---
# Formato: { "ibge": {"municipio": "...", "equipamento": "...", "categoria": "..."} }

# Mapeamento de municípios para IBGE (do dados_municipios.json existente)
...
EQUIPAMENTOS = [
    ...
    ("Guimarães", "2105604", "Retroescavadeira"),
    ...
    ("Lago Verde", "2106503", "Trator agrícola"),
    ...
    ("Matões", "2106557", "Trator agrícola"),
...
def main():
    ...
    # Build lookup: nome_upper → ibge
    nome_to_ibge = {}
    for m in d["municipios"]:
        nome_to_ibge[m["nome"].strip().upper()] = m["ibge"]

    # Build lookup: ibge → index
    ...
    for municipio_nome, ibge, equipamento in EQUIPAMENTOS:
        # Try IBGE lookup first
        idx = ibge_to_idx.get(ibge)
        if idx is not None:
            ...
            # Try name lookup
            norm_name = municipio_nome.strip().upper()
            found_ibge = nome_to_ibge.get(norm_name)
            if found_ibge:
                idx = ibge_to_idx[found_ibge]
                d["municipios"][idx]["equipamento_solicitado"] = equipamento
                d["municipios"][idx]["equipamento_categoria"] = (
                    "Trator agrícola" if "trator" in equipamento.lower() else "Retroescavadeira"
                )
                ...
                not_found.append(municipio_nome)
